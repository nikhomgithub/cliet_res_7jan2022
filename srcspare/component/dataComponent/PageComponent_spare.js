...
const { basicData,dataUrl,groupDataUrl,tableTemplateUrl,
...
const submitFunctionAdd=(formInputState)=>{
    ...
    addFileNameInPhotoUrl(tempFormInputState)    
    const fd=genFD({inputState:tempFormInputState,template:dataState})
...
return (
    ...
    <div style={{height:"100%",width:"100%",position:'relative'}}>
        ...
        ?<ModalForm
            lb={editFormTitle}
            formTemplate={dataEditForm}
            stateTemplate={dataState}
            loadData={{...editData,newId:editData.id}}
            selectData={{basicData:basicData}}
            setShow={setShowEdit}
            iconAction={iconAction}
            iconActionData={iconActionData}
